    ...
    if ($status -eq 200)
    {
        ...
        if($checkURLs)
        {
           ...
           foreach ($file in $files)
            {
                $filteredURLs,$bad,$counter=checkURLsFromFile -file $file -counter $counter
                    ...
                    if($f -eq "Default" -or $f.startsWith("Profile "))
                    {
                        ...
                        if (Test-Path "$baseDir\$f\Extensions")
                        {
                            ...
                            foreach ($r in $results)
                            {
                                ...
                                if($r.urls){foreach($u in $r.urls)
                                {
                                    if($u."status" -eq "ok")
                                    {
                                        write-host `t`t "URL: "  $u."url"  "  STATUS: "  $u."Malware Found" -ForegroundColor Red;
                                    }
                                    else
                                    {
                                        write-host `t`t "URL: "  $u."url"  "  STATUS: "  $u."OK"};
                                    }
                                    Write-Host
                                }
